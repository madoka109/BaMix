...
parser.add_argument('-im', '--imb_factor', default=0.005, type=float, help='imbalance factor')
parser.add_argument('--loss_type', default="BaMix", type=str, help='loss type')
parser.add_argument('--exp_str', default='0', type=str, help='number to ind   icate which experiment it is')
parser.add_argument('--train_rule', default='DRW', type=str, help='data sampling strategy for train loader')
parser.add_argument('--drw_epoch', default=320, type=int, metavar='N', help='defer re-weighting epoch')
...
parser.add_argument('--mixepoch', default=360, type=int)
...
def main():
    ...
    args.store_name = '_'.join([args.dataset, args.arch, args.loss_type, args.train_rule,
                                args.exp_str, str(args.imb_factor), 'alpha', str(args.al),'maxm',str(args.max_m),str(args.hp), 'mixepoch',str(args.mixepoch)])
    main_worker(args)
