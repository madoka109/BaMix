...
parser.add_argument('-a', '--arch', metavar='ARCH', default='resnet10i',
                    ...
                    help='model architecture: ' +
                        ' | '.join(model_names) +
                        ' (default: resnet10i)')
parser.add_argument('--dataset', default='iNaturalist', help='dataset setting')
parser.add_argument('--COLOR_SPACE', default='RGB', help='dataset setting')
parser.add_argument('--TRAIN_JSON', default='../data/iNat18/train2018.json', help='train dataset')
parser.add_argument('--VALID_JSON', default='../data/iNat18/val2018.json', help='valid setting')
...
parser.add_argument('--drw_epoch', default=240, type=int, metavar='N', help='defer re-weighting epoch')
...
parser.add_argument('--epochs', default=360, type=int, metavar='N', help='number of total epochs to run')
...
parser.add_argument('--devices', default='1', help='cuda device, i.e. 0 or 0,1,2,3')
parser.add_argument('--root_log',type=str, default='../log')
parser.add_argument('--root_model', type=str, default='../checkpoint')
parser.add_argument('--mixup', default=1, type=int)
parser.add_argument('--al', default=0.4, type=float)
parser.add_argument('--mixepoch', default=320, type=int)
parser.add_argument('--max_m', default=0.3, type=float)
parser.add_argument('--be', default=0.9999, type=float)
parser.add_argument('--use_AA', default='1', type=str, help='enter any character to use Auto-augment')
...
def main():
    ...
    args.store_name = '_'.join([args.dataset, args.arch, args.loss_type, args.train_rule,
                                args.exp_str, 'alpha', str(args.al), 'maxm',str(args.max_m), 'mixepoch',str(args.hp), 'hp', str(args.mixepoch)])
    main_worker(args)
